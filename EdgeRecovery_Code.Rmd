...
#--------------------------------------------------------------------------------#
# Calculate number of true ones

CountOnes <- function(true_data, thres_value){
  Counts <- matrix(ncol = length(thres_value), nrow = 1)
  #Adjacency of filtered gold data set
  for (x in 1:length(thres_value)){
    #print(paste("Loading:", paste0(data_comp_path, test_data[i])))
    #y <- load(paste0(data_comp_path, test_data[i]))
    #z <- get(y)
    
    #trueExpr_filtered <- true_data[rownames(z), colnames(z)]
    adj_true <- adjacency(true_data, type = "signed" , power = 9, corFnc = "bicor")
    ...
    threshold_gold <- mean(tom_true_melt$value) + (thres_value[x])*sd(tom_true_melt$value)
    ...
    print(paste("% of true edges=", sum(ref[,3])/dim(tom_true_melt)[1]*100))
    print(paste("Number of true edges=", sum(ref[,3])))
    Counts[1,x] <- sum(ref[,3])
  }
  colnames(Counts) <-c(paste0(thres_value,"std+mean"))
  rownames(Counts) <-"Gold"
  outputCounts <<- Counts
}
...
EvaluateSpecificity <- function(test_data, true_data, thres_value){
  ...
  ref <<- binarize(tom_true_red, threshold = threshold_gold)
  pred <- binarize(tom_test_red, threshold = threshold_gold)
  #print("Binarization done!")
...
CountOnes(true_data = trueExpr, thres_value = range, data_comp_path= rdata_input, test_data = data)
print(outputCounts)
```
...
```{r}
p <- df %>% 
  filter(EvalMetric =="Precision" | EvalMetric =="MCC" | EvalMetric == "Recall") %>%  filter(Threshold == "2xStd+mean") %>% 
  ggplot(aes(x =Source_Sparsity, y=value, colour=Imputation, group=Imputation))+
...
```{r}
load("01_BeMa_true_expression.RData")
adj_true <- adjacency(trueExpr, type = "signed" , power = 9, corFnc = "bicor")
tom_true <- WGCNA::TOMsimilarity(adj_true)
colnames(tom_true) <- colnames(adj_true)
rownames(tom_true) <- rownames(adj_true)
tom_true_melt <- melt(tom_true)

svg(paste0("R_02_", project,"_Dist_GoldR.svg"), width = 8, height = 6)

ggplot(tom_true_melt, aes(value))  + 
  geom_histogram(bins = 500) +
  coord_cartesian(ylim = c(0,500000), xlim = c(0, 0.1)) +
  geom_vline(xintercept = mean(tom_true_melt$value) + (1)*sd(tom_true_melt$value), col = "#0F0B77") +
  geom_vline(xintercept = mean(tom_true_melt$value) + (2)*sd(tom_true_melt$value), col = "#110ACB") +
  geom_vline(xintercept = mean(tom_true_melt$value) + (3)*sd(tom_true_melt$value), col = "#413BF1") +
  geom_vline(xintercept = mean(tom_true_melt$value) + (4)*sd(tom_true_melt$value), col = "#6F6BF9") +
  ...
  ylab("Frequency") +
  xlab("TOM value") 
  # labs(title="Distribution of Gold data correlation",
  #      subtitle = 'Showing different thresholds')
dev.off()


svg(paste0("R_02_", project,"_Dist_GoldR_full.svg"), width = 10, height = 8)
...
ggplot(tom_true_melt, aes(value))  + 
  ...
  ylab("Frequency") +
  xlab("TOM value") 
  # labs(title="Distribution of Topological Overlap Matrix Similarities",
  #      subtitle = 'Showing different thresholds')
dev.off()
